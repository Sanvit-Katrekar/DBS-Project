...
/*!50001 VIEW `availablebooks` AS select `book`.`book_id` AS `book_id`,`book`.`title` AS `title`,`book`.`author` AS `author`,(`book`.`total_copies` - count(`borrow`.`book_id`)) AS `copies_available` from (`book` left join `borrow` on(((`book`.`book_id` = `borrow`.`book_id`) and (`borrow`.`return_date` is null)))) group by `book`.`book_id` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddBook`(IN book_title VARCHAR(200), IN book_author VARCHAR(200), IN book_isbn VARCHAR(200), IN total_copies INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `BorrowBook`(IN book_id INT, IN student_id INT, IN borrow_date DATE, IN due_date DATE)
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `RemoveBook` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `RemoveBook`(IN book_id INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `ReturnBook`(IN brw_id INT, IN ret_date DATE)
BEGIN
